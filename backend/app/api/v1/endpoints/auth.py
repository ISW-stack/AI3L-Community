...
async def get_captcha(request: Request, type: str | None = None) -> CaptchaResponse:
    ...
    if type == "math":
        from app.services.captcha_math import generate_math_captcha

        captcha_id, image_base64 = await generate_math_captcha()
    else:
