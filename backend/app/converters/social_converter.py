...
def to_friendship_response(row: dict, current_user_id: str) -> FriendshipResponse:
...
def to_friend_request_response(row: dict) -> FriendRequestResponse:
    ...
    return FriendRequestResponse(
        ...
        requester_avatar_url=resolve_avatar_url(row.get("requester_avatar_url")),
        addressee_id=str(row["addressee_id"]),
        addressee_name=row["addressee_display_name"],
        addressee_username=row["addressee_username"],
        addressee_avatar_url=resolve_avatar_url(row.get("addressee_avatar_url")),
        status=row["status"],
        created_at=(
            row["created_at"].isoformat()
            if hasattr(row["created_at"], "isoformat")
...
def to_follow_user_response(row: dict) -> FollowUserResponse:
...
def to_block_response(row: dict) -> BlockResponse:
