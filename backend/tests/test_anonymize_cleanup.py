...
class TestAnonymizePostHistory:
    ...
    async def test_post_history_deleted(self, mock_pool, mock_conn):
        ...
        with (
            ...
            patch(
                "app.services.user.async_delete_file", new_callable=AsyncMock
            ),
        ):
        ...
        executed_sqls = [
            c.args[0] for c in mock_conn.execute.call_args_list if c.args
        ]
        assert any(
            "DELETE FROM post_history" in sql and "post_id" in sql
            for sql in executed_sqls
        ), "Expected DELETE FROM post_history for user's posts"

    async def test_post_history_deleted_before_posts_soft_delete(
...
class TestAnonymizeReactionsCleanup:
    ...
    async def test_reactions_cleaned_from_posts(self, mock_pool, mock_conn):
        ...
        assert any(
            "UPDATE posts" in sql
            and "reactions" in sql
            and "jsonb_array_elements_text" in sql
            for sql in executed_sqls
    ...
    async def test_reactions_cleaned_from_comments(self, mock_pool, mock_conn):
        ...
        assert any(
            "UPDATE comments" in sql
            and "reactions" in sql
            and "jsonb_array_elements_text" in sql
            for sql in executed_sqls
        ), "Expected reactions cleanup SQL for comments"

    async def test_reactions_cleanup_uses_user_id_string(
        self, mock_pool, mock_conn
    ):
        ...
        for c in mock_conn.execute.call_args_list:
            if c.args and "jsonb_array_elements_text" in str(c.args[0]):
                assert c.args[1] == str(
                    user_id
                ), "Reactions cleanup should pass user_id as string"
...
class TestAnonymizeMinIOCleanup:
    ...
    async def test_avatar_deleted_from_minio(self, mock_pool, mock_conn):
        ...
        with (
            ...
            mock_repo.find_by_id = AsyncMock(
                return_value={"avatar_url": avatar_key}
            )
            mock_repo.anonymize = AsyncMock(return_value=True)
            mock_conn.fetch = AsyncMock(return_value=[])
    ...
    async def test_dm_attachments_deleted_from_minio(
        self, mock_pool, mock_conn
    ):
    ...
    async def test_no_minio_deletion_when_no_avatar(
        self, mock_pool, mock_conn
    ):
    ...
    async def test_minio_deletion_failure_does_not_raise(
        self, mock_pool, mock_conn
    ):
    ...
    async def test_avatar_and_dm_files_both_deleted(
        self, mock_pool, mock_conn
    ):
        ...
        with (
            ...
            mock_repo.anonymize = AsyncMock(return_value=True)
            mock_conn.fetch = AsyncMock(
                return_value=[{"attachment_key": dm_key}]
            )
    ...
    async def test_no_minio_cleanup_when_anonymize_fails(
        self, mock_pool, mock_conn
    ):
