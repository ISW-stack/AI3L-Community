...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE posts ADD COLUMN type VARCHAR(30) NOT NULL DEFAULT 'post'"
    )
    op.execute(
        "ALTER TABLE posts ADD CONSTRAINT chk_post_type "
        "CHECK (type IN ('post', 'question'))"
    )
    op.execute(
    ...
    op.execute(
        "ALTER TABLE comments ADD COLUMN is_best_answer "
    ...
    op.execute(
        "ALTER TABLE notifications ALTER COLUMN action_type TYPE VARCHAR(50)"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE notifications ALTER COLUMN action_type TYPE VARCHAR(20)"
    )
