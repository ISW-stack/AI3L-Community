...
def upgrade() -> None:
    ...
    op.create_foreign_key(
        "form_responses_form_id_fkey", "form_responses", "forms",
        ["form_id"], ["id"], ondelete="CASCADE",
    )

    # comments.post_id → posts.id  ON DELETE CASCADE
    op.drop_constraint("comments_post_id_fkey", "comments", type_="foreignkey")
    op.create_foreign_key(
        "comments_post_id_fkey", "comments", "posts",
    ...
    op.create_foreign_key(
        "post_history_post_id_fkey", "post_history", "posts",
        ["post_id"], ["id"], ondelete="CASCADE",
    )
